using LeetCode;

Console.WriteLine("Hello, World!");

//int[] num1 = { 2, 4, 9 };
//int[] num2 = { 5, 6, 4, 9 };


//var solution = new AddTwoNumbers();
//solution.AddTwoNumbersListNode(solution.convertIntoList(num1), solution.convertIntoList(num2));


//LongestSubstring longestSubstring = new LongestSubstring();
//var result = longestSubstring.LengthOfLongestSubstring_bestSolution("dwdf");

//MedianOfTwoSortedArrays medianOfTwoSortedArrays = new MedianOfTwoSortedArrays();
//var result = medianOfTwoSortedArrays.FindMedianSortedArrays(new int[] {1,2},new int[] {3,4});
LongestPalindrome longestPalindrome = new LongestPalindrome();
var result = longestPalindrome.GetLongestPalindrome("abacc");
//var result = longestPalindrome.isPalindrome("vijay");
Console.WriteLine(result);
